#!/usr/bin/env python
# GiftSniper 0.9.8  —  night-sleep anytime 02:30-03:30 MSK for 3-7 h
...
# ─────────────── общие настройки ───────────────
VERBOSE             = True
NO_NEW_EVERY_SEC    = 60          # «новинок нет» не чаще, с
KEEPALIVE_PERIOD    = 90          # MTProto-ping период, с
RECONNECT_TRIES     = 5
RECONNECT_PAUSE     = 3           # между попытками, с

# окно начала сна (MSK)
SLEEP_START_FROM    = time(2, 30)     # 02:30
SLEEP_START_TO      = time(3, 30)     # 03:30
...
class GiftSniper:
    def __init__(self, cli: Client):
        ...
        if STORAGE.exists():
            try: self.seen |= {int(i) for i in json.loads(STORAGE.read_text())}
            except Exception as e: print("[WARN] gifts.json:", e)
    ...
    async def _fetch(self) -> List[Dict[str, Any]]:
        ...
        except InternalServerError:      # один ретрай
    ...
    # -------- public утилита для «check» --------
    async def list_gifts(self, only_new: bool):
        for g in await self._fetch():
            if only_new and g["id"] in self.seen:         # пропускаем уже виденные
                continue
            print(f"{g['title']:<4} | {g['price']:>7}⭐ | остаток: {g['supply']}")
        return

    # -------- рабочие функции --------
    def _eligible(self, g):
        return g["price"] and P_FROM<=g["price"]<=P_TO and g["supply"] and S_FROM<=g["supply"]<=S_TO
    ...
    async def tick(self):
        ...
        rare_i=bought_flag=new_flag=0

        for g in gifts:
            if g["id"] in self.seen: continue
            self.seen.add(g["id"]); new_flag=1

            max_q=10 if g["is_limited"] and rare_i==0 else 25 if g["is_limited"] and rare_i==1 else 0
            if g["is_limited"]: rare_i+=1

            if max_q and self._eligible(g) and BUY_GIFT:
                if await self._buy(g,max_q): bought_flag=1
...
async def reconnect(cli: Client):
...
def next_sleep_start()->datetime:
    ...
    delta=timedelta(minutes=random.randint(-30,+30))
    return base+delta     # 02:30-03:30

# ═════════════════════════════ main ═════════════════════════════
async def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--check",      action="store_true", help="показать только новые подарки и выйти")
    ap.add_argument("--check-all",  action="store_true", help="показать все подарки и выйти")
    args=ap.parse_args()
    ...
    try:
        ...
        if args.check or args.check_all:
            await sniper.list_gifts(only_new=not args.check_all)
            return

        # ---------------- основной режим ----------------
        ka=asyncio.create_task(keepalive(cli))
        ...
        while True:
            ...
            if now>=next_sleep:                     # периодический ночной сон
                dur_h=random.uniform(SLEEP_LEN_H_MIN,SLEEP_LEN_H_MAX)
                h,m=int(dur_h),int((dur_h-h)*60)
                print(f"😴 сон {h}ч{m:02d}м"); await asyncio.sleep(int(dur_h*3600))
                print("🌅 проснулись")
            ...
            try:
                ...
                if not await reconnect(cli):
                    break
                ka.cancel(); ka=asyncio.create_task(keepalive(cli))
            except Exception as e:
                print("[ERR] main:",e)
