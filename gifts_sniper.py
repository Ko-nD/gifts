#!/usr/bin/env python
# GiftSniper 1.0.1 – автопереподключение + исправленные timezone & finally
...
def fatal(m): print("[FATAL]", m); sys.exit(1)
if not (API_ID and API_HASH): fatal("TG_API_ID / TG_API_HASH отсутствуют")
...
class GiftSniper:
    ...
    async def _fetch(self):
        try:
            ...
            await asyncio.sleep(1); raw=await self.u.get_available_gifts()
    ...
    async def tick(self, show_all=False):
        gifts=await self._fetch()
        rare_i=bought=new=False
        if not isinstance(rare_i,bool): pass   # satisfy pylance
        rare_i=0; bought=False; new=False
        for g in gifts:
            if not show_all and g["id"] in self.seen: continue
            if not show_all: self.seen.add(g["id"]); new=True
            max_q = 10 if g["is_limited"] and rare_i==0 else \
                    25 if g["is_limited"] and rare_i==1 else 0
            if g["is_limited"]: rare_i+=1
            if max_q and self._eligible(g) and BUY_GIFT:
                if await self._buy(g,max_q): bought=True
            if show_all:
                print(f"{g['title']} | {g['price']}⭐ | {g['supply']} | lim={g['is_limited']}")
        if new:
...
async def reconnect(cli:Client)->bool:
    for n in range(1,RECONNECT_TRIES+1):
        try:
            ...
            print(f"[INFO] reconnect OK #{n}"); return True
        except Exception as e:
...
async def keepalive(cli:Client):
    while True:
        ...
        if not cli.is_connected: continue
        try:
...
def next_sleep_start():
    today=date.today()
    base=datetime.combine(today, time(3,0), ZoneInfo("Europe/Moscow"))
    return base+timedelta(minutes=random.randint(-30,30))
...
async def main():
    ...
    if args.check or args.check_all:
        await sniper.tick(show_all=args.check_all); await cli.stop(); return
    ...
    last_watchdog=datetime.now(ZoneInfo("Europe/Moscow"))
    ...
    try:
        while True:
            ...
            # sleep window
            if now>=next_sleep:
                dur=random.uniform(SLEEP_LEN_H_MIN,SLEEP_LEN_H_MAX)
                print(f"😴 сон {dur:.1f}ч"); await asyncio.sleep(int(dur*3600))
                print("🌅 проснулись")
            ...
            if (now-last_watchdog).seconds>=WATCHDOG_PERIOD:
                if not cli.is_connected:
                    print("[WARN] watchdog: reconnect…"); await reconnect(cli)
                last_watchdog=now
            try:
                await sniper.tick()
            except (OSError, ConnectionError) as e:
                print("[ERR] connection:",e); await reconnect(cli)
            await asyncio.sleep(random.randint(POLL_MIN,POLL_MAX))
    except AuthKeyDuplicated:
        fatal("AuthKeyDuplicated — сессия уже запущена.")
    finally:
        ka.cancel()
        await cli.stop()
